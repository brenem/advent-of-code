...
export class Day20 extends Challenge {
    async part1(input: string): Promise<PuzzleResult> {
        ...
        const cheatPaths = this.findCheatPaths(grid, tracks);

        const savings = cheatPaths.map((c) => ({
            cheatLocation: c.cheatLocation,
    ...
    private getTracksWithSteps(grid: Grid): {step: number, track: GridCoordinate}[] {
        const start = grid.find('S')!;
        let current = {step: 0, track: start};
        const tracks: {step: number, track: GridCoordinate}[] = [current];
        const visited = new Set<string>([`${current.track.location.x},${current.track.location.y}`]);
        let step = 0;

        while (true) {
            step++;

            const nextCell = this.getNextCell(grid, current.track, visited);
            const next = {step, track: nextCell};
            tracks.push(next);

            if (next.track.character === 'E') {
                break;
            }
    ...
    private findCheatPaths(
        ...
        for (const track of tracks) {
            ...
            const cheatLocations = this.getCheatCells(grid, track);
            ...
            if (cheatLocations.length > 0) {
                for (const cheatLocation of cheatLocations) {
                    ...
                    const tracksCountWithCheat = tracks.length - cheatIndex + 1;
